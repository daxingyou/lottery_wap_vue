    ...
    <div class="modal-content" v-show="loading">
      ...
      <ul>
        <li>
          <div>
            <span>{{groupName}}(</span>
            <span v-for="(item,i) in detail">{{item}}</span>
            <span>) ({{odds}})</span>
          </div>
          <div>
            <span class="monry">{{zonghe}}</span>
          </div>
        </li>
      </ul>
      <div class="total">
  ...
  props: {
    ...
    created(){
        ...
        for (let i=0; i<this.lotteryS.length; i++){
            ...
            if (i == this.lotteryS.length - 1) sbm = '';
            this.detail.push(this.lotteryS[i].key.substr(8,1)+sbm);
            this.odd[this.lotteryS[i].key] = this.money;
