...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < this.lotteryS.length; i++) {
      ...
      let z = this.lotteryS[i].key;
      let q = z.substr(3)
      let w = `${q},`
      this.xuanxiang = this.xuanxiang + s;
      this.odd.number = this.odd.number + w;
      this.groupName = this.lotteryS[i].groupName
      this.lengths++;
    }
    ...
    xiazhu() {
      ...
      this.$http.post(`${getUrl()}/inup`, JSON.stringify(this.odd), {}).then(res => {
        ...
        if((res.data) instanceof Array){
          ...
          this.sendParent();
          this.promptboxtext = "超出最大限额，最大限额是"+ res.data.maxbetamount;
          this.panelShow = true
          setTimeout(this.hideDailog, 1200);
        }
        else if (res.data.msg == 5001){
          this.sendParent();
          this.promptboxtext = "下注项为空或金额不正确，请重试"
          this.panelShow = true
          setTimeout(this.hideDailog, 1200);
          ...
          this.panelShow = true
          setTimeout(this.hideDailog, 1200);
        }else if(res.data.msg == 6001){
          this.sendParent();
          this.promptboxtext = "下注数量少于规则最小投注数量"
          this.panelShow = true
          setTimeout(this.hideDailog, 1200);
        }else if(res.data.msg == 6002){
          this.sendParent();
          this.promptboxtext = "下注数量大于规则最大投注数量"
          this.panelShow = true
          setTimeout(this.hideDailog, 1200);
