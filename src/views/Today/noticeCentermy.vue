  ...
  <div>
  	...
  	<div style="margin-top: 0.4rem;">
  		<div class="noticeCenter" v-for="(item,index) in annousments" @click="gotoAddress(index)" >
  			<span>{{getLocalTime(item.createdTime)}}</span><i v-if="item.read==0">NEW</i>
  			<p>{{item.message.slice(0,40)}}…</p>
  		</div>
  	</div>
  	<div class='noxiaoxi' v-show="total==0">
...
export default {
  ...
  created() {
    let params = {};
		this.STATION_LETTER().then(res => {
      this.annousments = res.data.response
      this.total = res.data.total
		})
  },
  methods: {
		...mapActions(["STATION_LETTER"]),
    gotoAddress(path) {
