	...
	<div>
		<div class="bj" @click='hideTitle'>
			<div class="hed color1" style="position: relative;">
				...
				<div style="display: inline-block;" @click.stop="showTitle">
					<span>{{menu_title}}</span>
					<i class="xiaoxia " style="float: right;margin-left: 0.2rem;"></i>
				</div>
			...
			<div class="biao" v-show="isReady">
				...
				<div class="kjh" v-for='(item,index) in lotteryList' :class="`game_${urlId}`">
					<div>
						...
						<span v-for='i in isNuber(item.number)' v-if="urlId==172" :class="`color_${i}`" class="cqsscdd">{{1}}</span>
						<a>{{timetrans(item.endtime)}}</a>
					</div>
...
export default {
  ...
  methods: {
    ...
    xgcsx(sx, round = "") {
      if (
        (this.urlId == 69 && round < 2018017) ||
        (this.urlId == 270 && round < 20180221000)
      ) {
  ...
  created() {
    ...
    if (sx_list) {
      ...
      this.$http
        .post(`/user/getMarkSixNumbers`, JSON.stringify(params))
        .then(res => {
...
.game_172 .cqsscdd:nth-of-type(1) {
  margin-left: -4rem;
}
