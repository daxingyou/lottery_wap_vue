    ...
    <article class="bjkl8-view">
        ...
        <gameChat>
          ...
          <section class="bet_bar"  ref="bet_bar">
            ...
            <div style='position:relative'><span style="color:#FFFFFF;" v-if="xshuzi>0">{{xshuzi}}</span><span style="background:`url(${getPublicImg('/images/small_m.png')}) no-repeat`;background-size:100% 100%;"  v-else="xshuzi=0">{{xshuzi}}</span>
...
import { bjkl8Classify } from "@/config/classify.config";
...
export default {
  ...
  beforeCreate() {
    this.bjkl8Classify = bjkl8Classify;
  },
  methods: {
      ...
      api
        ...
        .then(data => {
          ...
          if (data.data.msg == "4001") {
            ...
            if (this.activeClassifyId === 5000) {
                for (let i = 0; i < this.currentGame.length; i++) {
                  for(let j = 0; j < this.currentGame[i].length; j++) {
                    this.currentGame[i][j].isCheck = false;
                  }
                }
            }
  ...
  created() {
    ...
    if (oldTime && newTime <= oldTime && roundCha == 1) {
      ...
      let moneyX = JSON.parse(sessionStorage.getItem("im_money"));
        ...
        .then(res => {
          ...
          if (res.data.msg == "4001") {
            ...
            if (sessionStorage.getItem("im_realname") == "11") {
              ...
              sessionStorage.setItem(
                "im_money",
                JSON.stringify(res.data.lcurrency)
              );
            }
            this.round = res.data.next.round;
            this.roundCha =
