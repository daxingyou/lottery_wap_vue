...
<template>
  <div style='height:100%'>
    <dailog-q :type_code="activeClassifyId" :gametoken="gametoken" :game_code="240" :money="money_s" :round="round" :lotteryM="objects" v-if="showDailogQ" v-on:listenToChildEvent="showMsgFromChild"></dailog-q>
    ...
    <gameChat>
      <div style='overflow-x: hidden; overflow-y: auto; -webkit-overflow-scrolling: touch;width:100%;padding-bottom:2rem;'>
        <lotteryArea class="game_551" :lotteryObj="body" :zMoney="zMoney" :fenpan="fengpan" :endtime="endtime" :fentime="fentime" v-if="isOk" gameType="551"></lotteryArea>
        <div class="lottery_nav_bar" :style='de==true||loadpage==true?"display:none":"display:block"'>
          <ul>
            <button class="trapezoid color1" v-for="(item,i) in jsccClassify" :class="{active8: activeClassifyId === item.type_code}" @click="changeDate(item,i)">
              <span>{{item.name}}</span>
            </button>
          </ul>
        </div>
...
import api from "@/api";
import { jsccClassify } from "@/config/classify.config";
...
export default {
  ...
  created() {
    ...
    let oldTime = localStorage.getItem("jsc_time");
    let roundCha = localStorage.getItem("jsc_roundCha");
    let params = {
      oid: sessionStorage.getItem('im_token'),
      game_code: 240,
      type_code: 0
    ...
    if (oldTime && newTime <= oldTime && roundCha == 1) {
      let bodyS = localStorage.getItem("jsc_body");
      let oldTime = localStorage.getItem("jsc_time");
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.data.msg == "4001") {
            ...
            localStorage.setItem("jsc_time", loaclTime);
            localStorage.setItem("jsc_body", JSON.stringify(res.data));
            this.$refs.seller.className = "bet";
            this.numberList = res.data.last.number;
            this.round = res.data.next.round;
            this.roundCha =
              Number(res.data.next.round) - Number(res.data.last.round);
            localStorage.setItem("jsc_roundCha", JSON.stringify(this.roundCha));
          }
        })
        .catch(function() {
          this.$router.push({
        ...
        this.$http
          ...
          .then(res => {
            ...
            if (res.data.msg == 4001) {
              ...
              localStorage.getItem("jsc_roundCha");
            }
          });
