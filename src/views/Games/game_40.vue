<template>
  <div style='height:100%'>
    ...
    <div id="cont_main" v-on:touchmove="banmove()" style='position:absolute;top:0;bottom:2.45rem;    overflow: auto; -webkit-overflow-scrolling: touch;width:100%'>
      <lotteryArea :lotteryObj="body" :endtime="endtime" :fenpan="!!fengpan" :fentime="fentime" v-if="isOk" gameType="40"></lotteryArea>
      <div :style='de==true&&loadpage==false?"display:none":"display:block"' class="lottery_nav_bar">
        <ul>
          <button class="trapezoid color1" v-for="(item,i) in cqsscClassify" :class="{active8: activeTabIndex === item.type_code}" @click="changeTab(item,i)">
            <span>{{item.name}}</span>
          </button>
        </ul>
      </div>
