...
export default {
  ...
  created() {
    ...
    let oldTime = localStorage.getItem("tj_time");
    let roundCha = localStorage.getItem("tj_roundCha");
    if (oldTime && newTime <= oldTime && roundCha == 1) {
      let moneyX = sessionStorage.getItem("im_money");
      this.zMoney = parseFloat(moneyX);
      let bodyS = localStorage.getItem("tj_body");
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.data.msg == "4001") {
            ...
            localStorage.setItem("tj_time", loaclTime);
            localStorage.setItem("tj_body", JSON.stringify(res.data));
            this.numberList = res.data.last.number;
            this.round = res.data.next.round;
            this.roundCha =
              Number(res.data.next.round.split("-")[1]) -
              Number(res.data.last.round.split("-")[1]);
            localStorage.setItem("tj_roundCha", JSON.stringify(this.roundCha));
          }
        })
        .catch(function() {
          this.$router.push({
