  ...
  <div style='height:100%'>
    ...
    <gameChat>
      <div style='overflow-x: hidden; overflow-y: auto; -webkit-overflow-scrolling: touch;width:100%;padding-bottom:2rem;'>
        <lotteryArea :lotteryObj="body" :endtime="endtime" :zMoney="zMoney" :fenpan="fengpan" :fentime="fentime" v-if="isOk" gameType="60"></lotteryArea>
        <div :style='de==true&&loadpage==false?"display:none":"display:block"' class="lottery_nav_bar">
          <ul>
            <button class="trapezoid color1" v-for="(item,i) in jssscClassify" :class="{active8: activeClassifyId === item.type_code}" @click="changeDate(item,i)">
              <span>{{item.name}}</span>
            </button>
          </ul>
        </div>
...
export default {
  ...
  created() {
    ...
    let oldTime = localStorage.getItem("jssc_time");
    let roundCha = localStorage.getItem("jssc_roundCha");
    if (oldTime && newTime <= oldTime && roundCha == 1) {
      let moneyX = sessionStorage.getItem("im_money");
      this.zMoney = parseFloat(moneyX);
      let bodyS = localStorage.getItem("jssc_body");
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.data.msg == "4001") {
            ...
            localStorage.setItem("jssc_time", loaclTime);
            localStorage.setItem("jssc_body", JSON.stringify(res.data));
