...
<template>
  <div style='height:100%'>
    ...
    <lotteryHeader :title="title" :game_code=210></lotteryHeader>
    <div style='position:absolute;top:0;bottom:2.45rem;    overflow: auto; -webkit-overflow-scrolling: touch;width:100%'>
      <lotteryArea class="game_55" :lotteryObj="body" :fenpan="!!fengpan" :endtime="endtime" :fentime="fentime" v-if="isOk" gameType="66"></lotteryArea>
      <div class="lottery_nav_bar" :style='de==true||loadpage==true?"display:none":"display:block"'>
        <ul>
          <button class="trapezoid color1" v-for="(item,i) in wnsClassify" :class="{active8: activeClassifyId === item.type_code}" @click="changeTab(item,i)">
            <span>{{item.name}}</span>
          </button>
        </ul>
      </div>
