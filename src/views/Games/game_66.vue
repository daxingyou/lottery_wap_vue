...
<template>
  <div style='height:100%'>
    <dailog-q :gametoken="gametoken" :type_code="activeClassifyId" :game_code="210" :money="money_s" :round="round" :lotteryM="objects" v-if="showDailogQ" v-on:listenToChildEvent="showMsgFromChild"></dailog-q>
    <lotteryHeader :title="title" :game_code=210 @regulation_click="regulation_control=true"></lotteryHeader>
    <div style='position:absolute;top:0;bottom:2.45rem;    overflow: auto; -webkit-overflow-scrolling: touch;width:100%'>
      <lotteryArea class="game_55" :lotteryObj="body" :zMoney="zMoney" :fenpan="fengpan" :endtime="endtime" :fentime="fentime" v-if="isOk" gameType="66"></lotteryArea>
      <div class="lottery_nav_bar" :style='de==true||loadpage==true?"display:none":"display:block"'>
        <ul>
          <button class="trapezoid color1" v-for="(item,i) in wnsClassify" :class="{active8: activeClassifyId === item.type_code}" @click="changeDate(item,i)">
            <span>{{item.name}}</span>
          </button>
        </ul>
      </div>
...
import api from "@/api";
import { wnsClassify } from "@/config/classify.config";
...
export default {
  ...
  created() {
    ...
    let oldTime = localStorage.getItem("wns_time");
    let roundCha = localStorage.getItem("wns_roundCha");
    let params = {
      oid: sessionStorage.getItem('im_token'),
      game_code: 210,
      type_code: 0
    ...
    if (oldTime && newTime <= oldTime && roundCha == 1) {
      let bodyS = localStorage.getItem("wns_body");
      let oldTime = localStorage.getItem("wns_time");
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.data.msg == "4001") {
            ...
            localStorage.setItem("wns_time", loaclTime);
            localStorage.setItem("wns_body", JSON.stringify(res.data));
            this.$refs.seller.className = "bet";
            this.numberList = res.data.last.number;
            this.round = res.data.next.round;
            this.roundCha =
              Number(res.data.next.round) - Number(res.data.last.round);
            localStorage.setItem("wns_roundCha", JSON.stringify(this.roundCha));
          }
        })
        .catch(function() {
          this.$router.push({
        ...
        this.$http
          ...
          .then(res => {
            ...
            if (res.data.msg == 4001) {
              ...
              localStorage.getItem("wns_roundCha");
            }
          });
