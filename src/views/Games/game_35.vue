<template>
  <div style='height:100%'>
    ...
    <div style='position:absolute;top:0;bottom:2.45rem;    overflow: auto; -webkit-overflow-scrolling: touch;width:100%'>
      <lotteryArea class="game_35" :lotteryObj="body" :zMoney="zMoney" :fenpan="fengpan" :endtime="endtime" :fentime="fentime" v-if="isOk" gameType="35"></lotteryArea>

      <div class="lottery_nav_bar" :style='de==true||loadpage==true?"display:none":"display:block"'>
        <ul>
          <button class="trapezoid color1" v-for="(item,i) in xyftClassify" :class="{active8: activeClassifyId === item.type_code}" @click="changeDate(item,i)">
            <span>{{item.name}}</span>
          </button>
        </ul>
      </div>
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.data.msg == "4001") {
            ...
            this.round = res.data.next.round;
            this.roundCha =
              Number(this.body.next.round) - Number(this.body.last.round);
