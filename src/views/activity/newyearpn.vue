  ...
  export default {
    ...
    methods: {
     ...
     ljlq(){
		...
		this.$http.post(`/DepositActivity/getBonusesYesterday`, JSON.stringify(params)).then(res =>{
       		this.promptboxshow =false
    ...
    created(){
		...
		if(window.location.href.indexOf('?')!=-1){
			...
			this.$http.post(`/DepositActivity/getDepositYesterday`, JSON.stringify(params)).then(res =>{
...
.container {
  ...
  .title{
	...
	#nowyear{
		width: 100%;
		height: 1810/46.875rem;
		background: url(../../../wap/images/pn/newyear.png) no-repeat;
		background-size: 100% 100%;
		position: relative;
		>.intermediately{
			position: absolute;
