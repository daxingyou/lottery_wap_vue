<template>
  ...
  <div>
    <z-header title="电子记录"></z-header>
    <div class="content">
      <div @click='quanbu' v-if='isWan!="游客"' :class="{active:isShow}">汇总</div>
    </div>
		<div class="centermiddle">
    <div class="list3" v-for="item in history" v-show="isShow4" @click="yijie(item.dateTime)">
      <div>
        ...
        <div>
          ...
          <span>
              <i style="margin-left:.4rem">下注金额</i>
              <i>{{item.gold}}</i>
          </span>
        </div>
        <div>
          ...
          <span>
              <i style="margin-left:.4rem">输赢结果</i>
              <i>{{item.type}}</i>
          </span>
        </div>
      </div>
