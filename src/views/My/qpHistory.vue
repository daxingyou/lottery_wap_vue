        ...
        <div>
          ...
          <div class="list5" v-for="its in item.content">
            <div>
              ...
              <span>
                <i style="margin-left:.4rem">游戏总局数</i>
                <i>{{its.nums}}</i>
              </span>
            </div>
            <div>
              ...
              <span>
                <i style="margin-left:.4rem">总抽税</i>
                <i>{{its.inc_rev}}</i>
              </span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    pagenumb(index) {
      ...
      this.$http
        .post(`/Wh_H5_Api/SearchGameRecord`, JSON.stringify(prams))
        .then(res => {
          if (this.isShow3) {
