        ...
        <div class='bgImg' ref="bgagimg">
          ...
          <div class="edzhmodel" v-show="isEdzh">
            <div class="edzhdiv">
              ...
              <div class="edzhFood">
                <label><input type="radio" class="radio" name="1"  v-model="transferType" value="1"/><i class="checkboxinput"></i><a>彩票余额</a><img :src="$getPublicImg('/images/edzhjt.png')"/><a>体育余额</a></label>
                <label>
                  <input type="radio" class="radio" v-model="transferType" value="0"/>
                  <i class="checkboxinput"></i><a>体育余额</a>
  ...
  export default {
    ...
    methods:{
    	...
    	agzc(){
         ...
         if (this.isIndex==1) {
           ...
           this.$router.go(-1)
			...
			// ykzr(){
			// 	window.open("http://cptyguan.lebole2.com")
			// },
    	ykzr(){
    		...
    		if(this.isWan=='游客'){
    			...
    			if(Number(this.sportMoney)==0){
						this.panelShow = true;
		        this.promptboxtext = '体育余额不足'
    			}else{
    				this.showCurtion=true
    			let params ={};
		        params.oid = sessionStorage.getItem('im_token');
		        this.$http.post(getUrl()+'/SportFunction/logToSport',JSON.stringify(params)).then(res => {
							// return false;
    	...
    	gbedzh(){
      	...
      	submit(){
	        ...
	        params.amount = this.nut
					...
					console.log(params.transfer_io)
				// kong 或者 0
	        if(this.nut ==null){
	          this.panelShow = true;
	        ...
	        if(this.transferType == 1){// 彩票转体育
	          ...
	          if(Number(this.sportMoney) < Number(this.nut)){
	            this.panelShow = true;
	            this.promptboxtext = '体育余额不足';
	            this.$refs.rscenter.style.backgroundImage = "url('../../../wap/images/erreo.png')";
	            setTimeout(()=>{
      ...
      getData(params){
      	...
      	// this.successshow=false
      		this.$http.post(getUrl()+'/user/conversionCash',JSON.stringify(params)).then(res => {
   				this.showCurtion = false;
          if(res.data.msg == 4001){
            sessionStorage.clear();
            this.panelShow = true;
            ...
            this.$refs.rscenter.style.backgroundImage = "url('../../../wap/images/erreo.png')"
            if(this.nut.length == 0){
              this.promptboxtext = "转账金额应为大于1的正整数";
              this.panelShow = true;
              setTimeout(() => {
                this.panelShow = false;
              },1200)
            ...
            if(this.transferType == 1){
              this.sportMoney = parseFloat(this.sportMoney) +  parseFloat(res.data.amount);
            ...
            this.sportMoney = this.sportMoney.toFixed(2);
          ...
          if (!sessionStorage.getItem("im_token")) {
           this.$router.push("/login")
             ...
             this.$router.push(path)
				...
				this.$router.push('/sportHistory')
      },
      gotoPayAddresscz(path){
				 this.$router.push('/order:2?GameName=sport')
      },
    ...
    created() {
       	...
       	let params = {};
     		params.oid = sessionStorage.getItem('im_token');
				this.$http.post(`${getUrl()}/user/getSportsBalance`, JSON.stringify(params)).then(res => {
				this.showCurtion = false;
				 if(res.data.msg == 4001){
			          sessionStorage.clear();
			          this.panelShow = true;
			          ...
			          this.sportMoney = Number(res.data.balance).toFixed(2);
			          this.balance = Number(res.data.userBalance).toFixed(2);
			          sessionStorage.setItem('im_money', this.balance)
		        }else if(res.data.msg == 7001){
			          this.promptboxtext = res.data.info;
			          this.panelShow = true;
...
.box{
  ...
  -webkit-overflow-scrolling: touch;
    ...
    height: 100%;
		background: url(../../../wap/images/sport.jpg) no-repeat;
		background-size: 100% 100%;
		/*position:fixed;*/
		/*top:0;*/
		>.edzh{
