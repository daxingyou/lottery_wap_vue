		...
		<div class='bgImg' ref="bgagimg">
			...
			<div class="edzhmodel" v-show="isEdzh">
				<div class="edzhdiv">
					...
					<div class="edzhFood">
						<label><input type="radio" class="radio" name="1" v-model="transferType" value="1" />
							<i class="checkboxinput"></i>
							<a>彩票余额</a><img :src="$getPublicImg('/images/edzhjt.png')" />
							<a>体育余额</a>
						</label>
						<label>
							...
							<i class="checkboxinput"></i>
							<a>体育余额</a>
							<img :src="$getPublicImg('/images/edzhjt.png')" />
							<a>彩票余额</a>
						</label>
						<div class="numberMoney">
							<span></span>
							<input type="tel" pattern="\d*" placeholder="请输入转换金额" @input.stop.prevent="zhengshu" v-model="nut" />
						</div>
						<div class="edzhbtn">
  ...
  methods: {
    ...
    ykzr() {
      ...
      if (this.isWan == "游客") {
        ...
        if (Number(this.sportMoney) == 0) {
          ...
          this.$http
            .post(
              getUrl() + "/SportFunction/logToSport",
    ...
    submit() {
      ...
      if (this.nut == null) {
        ...
        this.$refs.rscenter.style.backgroundImage =
          "url('../../../wap/images/erreo.png')";
        setTimeout(() => {
          this.panelShow = false;
        }, 1200);
        return;
    ...
    getData(params) {
      ...
      // this.successshow=false
      this.$http
        .post(getUrl() + "/user/conversionCash", JSON.stringify(params))
        .then(res => {
          this.showCurtion = false;
          if (res.data.msg == 7001) {
            ...
            if (this.transferType == 1) {
              this.sportMoney =
                parseFloat(this.sportMoney) + parseFloat(res.data.amount);
              this.balance =
                parseFloat(this.balance) - parseFloat(res.data.amount);
